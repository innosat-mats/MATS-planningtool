...
def SNAPSHOT(Occupied_Timeline, configFile):
    "Plan to schedule the Mode at the start of the timeline"
    initial_StartDate = DT.datetime.strptime(configFile.Timeline_settings()['start_date'],'%Y/%m/%d %H:%M:%S')

    "Set a duration of the Mode in seconds, which in turns sets the endDate"
    #duration = 600
    duration = configFile.SNAPSHOT_settings()['duration']
    endDate = initial_StartDate + DT.timedelta(seconds = duration)
